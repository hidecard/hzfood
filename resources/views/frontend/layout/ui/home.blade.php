    ...
    <div id="carouselExampleIndicators" class="carousel slide" data-bs-ride="true">
        ...
        <div class="carousel-inner">
          <div class="carousel-item active">
            <img src="{{asset('img/gg.png')}}" class="d-block w-100" alt="...">
          </div>
              ...
              <div class="col-lg-3 col-md-6 mb-4 mx-1 d-flex justify-content-center">
                <div id="menuitem" class="services-wrap text-center shadow p-3 mb-5 bg-body">
                    <div class="position-relative">
                        <div class="{{ $item->stock == 0 ? 'outstock' : 'instock' }}">
                            <i class="fa-solid fa-circle fs-4 m-2"></i>
                        </div>
                      </div>
                      {{-- <p>{{asset('storage/images/' .$item->img)}}</p> --}}
                      <img id="imagemenu" src="{{asset('storage/images/' .$item->img)}}" alt="">
                      <div class="text">
                        <h3 class="bg-light">{{$item->name}}</h3>
                        <span class="d-flex justify-content-center text-center bg-light {{ $item->stock == 0 ? 'text-danger' : 'text-success' }}">
                            <i class="fa-solid fa-circle mx-2 bg-light"></i>
                            <h6 class="bg-light">{{ $item->stock == 0 ? 'Out Of Stock' : 'In Stock' }}</h6>
                        </span>
                            <div class="bg-light">{{$item->item_price}} MMK</div>
                            @if ($item->stock != 0)
                            @if (session('user_id'))
                            ...
                            @else
                            <a href="{{ route('login') }}" class="btn btn-outline-danger mt-2">Login to Add to Cart</a>
                            @endif
                            @endif
                    </div>
                </div>
